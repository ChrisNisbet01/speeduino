...
static inline void
pin_set(volatile PORT_TYPE &port, volatile PINMASK_TYPE mask)
{
...
static inline void
pin_clear(volatile PORT_TYPE &port, volatile PINMASK_TYPE mask)
{
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
  {
    port &= ~mask;
...
#define AIRCON_ON() do { \
  ...
  BIT_SET(currentStatus.airConStatus, BIT_AIRCON_COMPRESSOR); \
  } while(0)
