...
//!< The time at which the serial receive started. Used for calculating whether a timeout has occurred */
...
{
  return millis() - serialReceiveStartTime > SERIAL_TIMEOUT_MS;
}
 ...
 *
 * @attention noinline is needed to prevent enlarging caller's stack frame,
 * which in turn throws off free ram reporting.
 * */
static __attribute__((noinline)) uint32_t reverse_bytes(uint32_t i)
{
  return ((i >> 24) & 0x000000ffU) // move byte 3 to byte 0
         | ((i << 8) & 0x00ff0000U)  // move byte 1 to byte 2
         | ((i >> 8) & 0x0000ff00U)  // move byte 2 to byte 1
         | ((i << 24) & 0xff000000U);
}
...
void writeByteReliableBlocking(byte value)
{
  ...
  while (!Serial.availableForWrite()) //cppcheck-suppress misra-c2012-17.2
  {
...
static __attribute__((noinline)) uint32_t readSerial32Timeout(void)
{
  ...
  while (count < sizeof(buffer.value))
  {
    ...
    else //Flag the serial receive as being in progress
    {
      /* MISRA - no-op */
    }
  }
...
{
  writeByteReliableBlocking((value >> 8U) & 0xff);
  writeByteReliableBlocking(value & 0xff);
}
...
static void generateLiveValues(uint16_t offset, uint16_t packetLength)
{
  ...
  //Set the sync bit of the Spark variable to match the hasSync variable
  currentStatus.spark ^= (-currentStatus.hasSync ^ currentStatus.spark) & (1U << BIT_SPARK_SYNC);
...
static uint16_t fahrenheit_to_c(uint16_t f)
{
   //Convert from F to C
  return ((f - 32) * 5) / 9;
}
...
static uint16_t toTemperature(byte lo, byte hi)
{
  ...
  tempValue = fahrenheit_to_c(tempValue);
 ...
 */
static void processTemperatureCalibrationTableUpdate(
  uint16_t calibrationLength,
...
void serialReceive(void)
{
  ...
  if (Serial.available() != 0 && serialStatusFlag == SERIAL_INACTIVE)
  {
    ...
    else if (((highByte >= 'A' && highByte <= 'z') || (highByte == '?'))
    ...
    else
    {
      ...
      while (Serial.available() == 0)
      {
        /* Wait for the 2nd byte to be received
         * (This will almost never happen, as the 2nd byte will usually be present)
         */
      }
  ...
  //If there is a serial receive in progress, read as much from the buffer as
  //possible or until we receive all bytes
    ...
    else
    {
      ...
      if (!isTimeout()) // CRC read can timeout also!
      {
        if (incomingCrc == CRC32_serial.crc32(serialPayload, serialPayloadLength))
        {
          ...
          //Lock out legacy commands until next power cycle
...
void serialTransmit(void)
{
  ...
  case SERIAL_TRANSMIT_INPROGRESS:
    ...
    serialStatusFlag =
      (serialBytesRxTx == serialPayloadLength + sizeof(crc_t))
      ? SERIAL_INACTIVE
...
void processSerialCommand(void)
{
  ...
  case 'B':
    // Same as above, but for the comms compat mode.
    // Slows down the burn rate and increases the defer time
  ...
  {
    //Get the CRC for the requested page
  ...
  case 'r': //New format for the optimised OutputChannels
  {
    ...
    else if (cmd == SD_READWRITE_PAGE) //Request SD card extended parameters
    {
      ...
      if (SD_arg1 == SD_READ_STAT_ARG1 && SD_arg2 == SD_READ_STAT_ARG2)
      {
        ...
        serialPayload[5] = ((sectors >> 24) & 0xff);
        serialPayload[6] = ((sectors >> 16) & 0xff);
        serialPayload[7] = ((sectors >> 8) & 0xff);
        serialPayload[8] = (sectors & 0xff);

        //Max roots (Number of files)
        // -1 because this returns the NEXT file name not the current one
        // and -1 because TS expects a 0 based index.
      ...
      else if (SD_arg1 == SD_READ_DIR_ARG1 && SD_arg2 == SD_READ_DIR_ARG2)
      {
        ...
        while (filesInCurrentChunk < 16
               && getSDLogFileDetails(&serialPayload[payloadIndex], logFileNumber))
        {
          logFileNumber++;
          filesInCurrentChunk++;
          payloadIndex += 32;
  ...
  case 't': // receive new Calibration info. Command structure: "t", <tble_idx> <data array>.
  {
    ...
    else if (cmd == IAT_CALIBRATION_PAGE)
    {
      processTemperatureCalibrationTableUpdate(
        calibrationLength, IAT_CALIBRATION_PAGE, iatCalibration_values, iatCalibration_bins);
    }
    else if (cmd == CLT_CALIBRATION_PAGE)
    {
      processTemperatureCalibrationTableUpdate(
        calibrationLength, CLT_CALIBRATION_PAGE, cltCalibration_values, cltCalibration_bins);
    }
    else
    {
      sendReturnCodeMsg(SERIAL_RC_RANGE_ERR);
  ...
  case 'U': //User wants to reset the Arduino (probably for FW update)
    if (resetControl != RESET_CONTROL_DISABLED)
    {
      ...
      {
        Serial.println(F("Comms halted. Next byte will reset the Arduino."));
      }
#endif
...
#     ifndef SMALL_FLASH_MODE
      if (serialStatusFlag == SERIAL_INACTIVE)
      {
        Serial.println(F("Reset control is currently disabled."));
      }
#     endif
    }
    break;
    ...
    if (cmd == SD_READWRITE_PAGE)
    {
      if (SD_arg1 == SD_WRITE_DO_ARG1 && SD_arg2 == SD_WRITE_DO_ARG2)
      {
        ...
        */
        ...
        if (command == 2)
        {
          endSDLogging(); manualLogActive = false;
        }
        else if (command == 3)
        {
          beginSDLogging(); manualLogActive = true;
        }
        else if (command == 4)
        {
          setTS_SD_status();
      ...
      {
        //Read sector Init? Unsure what this is meant to do however it is sent
        //at the beginning of a Card Format request and requires an OK response
        //Provided the sector being requested is x0 x0 x0 x0, we treat this as an
...
void sendToothLog(void)
{
  //We need TOOTH_LOG_SIZE number of records to send to TunerStudio.
  //If there aren't that many in the buffer then we just return and wait for the next call
  ...
  if (logItemsTransmitted == 0)
  {
    ...
    //Size of the tooth log (uint32_t values) plus the return code
...
void sendCompositeLog(void)
{
  ...
  for (; logItemsTransmitted < TOOTH_LOG_SIZE; logItemsTransmitted++)
  {
    ...
    //This combined runtime (in us) that the log was going for by this record
    uint32_t transmitted = serialWrite(toothHistory[logItemsTransmitted]);
    ...
    CRC32_val = CRC32_serial.crc32_upd(
      (const byte *)&compositeLogHistory[logItemsTransmitted],
      sizeof(compositeLogHistory[logItemsTransmitted]),
      false);
  }
  BIT_CLEAR(currentStatus.status1, BIT_STATUS1_TOOTHLOG1READY);
  toothHistoryIndex = 0;
