...
void receiveCANwbo()
{
  ...
  if(configPage2.canWBO == CAN_WBO_RUSEFI)
  {
    ...
    // Enable heater once engine is running (ie. above cranking rpm),
    // this condition can be changed to CLT above certain temp and so on.
  ...
  if (PIDmode == 0x01)
  {
    switch (requestedPIDlow)
    {
      ...
      case 17:  // PID-0x11 ,
        ...
        obdcalcA = ((uint16_t)currentStatus.TPS << 8) / 100;     // (tpsPC *256) /100;
