...
byte getTSLogEntry(uint16_t byteNum)
{
  ...
  switch(byteNum)
  {
    ...
    case 76: statusValue = lowByte(injectors.injector(injChannel1).PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 77: statusValue = highByte(injectors.injector(injChannel1).PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 78: statusValue = lowByte(injectors.injector(injChannel2).PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 79: statusValue = highByte(injectors.injector(injChannel2).PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 80: statusValue = lowByte(injectors.injector(injChannel3).PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 81: statusValue = highByte(injectors.injector(injChannel3).PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 82: statusValue = lowByte(injectors.injector(injChannel4).PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
    case 83: statusValue = highByte(injectors.injector(injChannel4).PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
...
int16_t getReadableLogEntry(uint16_t logIndex)
{
  ...
  switch(logIndex)
  {
    ...
    case 53: statusValue = injectors.injector(injChannel1).PW; break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 54: statusValue = injectors.injector(injChannel2).PW; break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 55: statusValue = injectors.injector(injChannel3).PW; break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 56: statusValue = injectors.injector(injChannel4).PW; break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
...
float getReadableFloatLogEntry(uint16_t logIndex)
{
  ...
  switch(logIndex)
  {
    ...
    case 53: statusValue = injectors.injector(injChannel1).PW / 1000.0; break; //Pulsewidth 1 Have to convert from uS to mS.
    case 54: statusValue = injectors.injector(injChannel2).PW / 1000.0; break; //Pulsewidth 2 Have to convert from uS to mS.
    case 55: statusValue = injectors.injector(injChannel3).PW / 1000.0; break; //Pulsewidth 3 Have to convert from uS to mS.
    case 56: statusValue = injectors.injector(injChannel4).PW / 1000.0; break; //Pulsewidth 4 Have to convert from uS to mS.
...
uint8_t getLegacySecondarySerialLogEntry(uint16_t byteNum)
{
  ...
  switch(byteNum)
  {
    ...
    case 20: statusValue = lowByte(injectors.injector(injChannel1).PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 21: statusValue = highByte(injectors.injector(injChannel1).PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    ...
    case 76: statusValue = lowByte(injectors.injector(injChannel2).PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 77: statusValue = highByte(injectors.injector(injChannel2).PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 78: statusValue = lowByte(injectors.injector(injChannel3).PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 79: statusValue = highByte(injectors.injector(injChannel3).PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 80: statusValue = lowByte(injectors.injector(injChannel4).PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
    case 81: statusValue = highByte(injectors.injector(injChannel4).PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
...
void startToothLogger(void)
{
  ...
  detachInterrupt(digitalPinToInterrupt(Trigger.getPin()));
  attachInterrupt(digitalPinToInterrupt(Trigger.getPin()), loggerPrimaryISR, CHANGE);

  if (!VSS_USES_RPM2())
  {
    detachInterrupt(digitalPinToInterrupt(Trigger2.getPin()));
    attachInterrupt(digitalPinToInterrupt(Trigger2.getPin()), loggerSecondaryISR, CHANGE);
  }
...
void stopToothLogger(void)
{
  ...
  detachInterrupt(digitalPinToInterrupt(Trigger.getPin()));
  attachInterrupt(digitalPinToInterrupt(Trigger.getPin()), triggerHandler, primaryTriggerEdge);

  if (!VSS_USES_RPM2())
  {
    detachInterrupt(digitalPinToInterrupt(Trigger2.getPin()));
    attachInterrupt(digitalPinToInterrupt(Trigger2.getPin()), triggerSecondaryHandler, secondaryTriggerEdge);
  }
}
...
void startCompositeLoggerTertiary(void)
{
  ...
  detachInterrupt(digitalPinToInterrupt(Trigger3.getPin()) );
  attachInterrupt(digitalPinToInterrupt(Trigger3.getPin()), loggerTertiaryISR, CHANGE);
}
...
void stopCompositeLoggerTertiary(void)
{
  ...
  detachInterrupt(digitalPinToInterrupt(Trigger3.getPin()));
  attachInterrupt(digitalPinToInterrupt(Trigger3.getPin()), triggerTertiaryHandler, tertiaryTriggerEdge);
}
