...
byte getTSLogEntry(uint16_t byteNum)
{
  ...
  switch(byteNum)
  {
    ...
    case 76: statusValue = lowByte(injectors_context.injectors[0].PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 77: statusValue = highByte(injectors_context.injectors[0].PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 78: statusValue = lowByte(injectors_context.injectors[1].PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 79: statusValue = highByte(injectors_context.injectors[1].PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 80: statusValue = lowByte(injectors_context.injectors[2].PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 81: statusValue = highByte(injectors_context.injectors[2].PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 82: statusValue = lowByte(injectors_context.injectors[3].PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
    case 83: statusValue = highByte(injectors_context.injectors[3].PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
...
int16_t getReadableLogEntry(uint16_t logIndex)
{
  ...
  switch(logIndex)
  {
    ...
    case 53: statusValue = injectors_context.injectors[0].PW; break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 54: statusValue = injectors_context.injectors[1].PW; break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 55: statusValue = injectors_context.injectors[2].PW; break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 56: statusValue = injectors_context.injectors[3].PW; break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
...
float getReadableFloatLogEntry(uint16_t logIndex)
{
  ...
  switch(logIndex)
  {
    ...
    case 53: statusValue = injectors_context.injectors[0].PW / 1000.0; break; //Pulsewidth 1 Have to convert from uS to mS.
    case 54: statusValue = injectors_context.injectors[1].PW / 1000.0; break; //Pulsewidth 2 Have to convert from uS to mS.
    case 55: statusValue = injectors_context.injectors[2].PW / 1000.0; break; //Pulsewidth 3 Have to convert from uS to mS.
    case 56: statusValue = injectors_context.injectors[3].PW / 1000.0; break; //Pulsewidth 4 Have to convert from uS to mS.
...
uint8_t getLegacySecondarySerialLogEntry(uint16_t byteNum)
{
  ...
  switch(byteNum)
  {
    ...
    case 20: statusValue = lowByte(injectors_context.injectors[0].PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    case 21: statusValue = highByte(injectors_context.injectors[0].PW); break; //Pulsewidth 1 multiplied by 10 in ms. Have to convert from uS to mS.
    ...
    case 76: statusValue = lowByte(injectors_context.injectors[1].PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 77: statusValue = highByte(injectors_context.injectors[1].PW); break; //Pulsewidth 2 multiplied by 10 in ms. Have to convert from uS to mS.
    case 78: statusValue = lowByte(injectors_context.injectors[2].PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 79: statusValue = highByte(injectors_context.injectors[2].PW); break; //Pulsewidth 3 multiplied by 10 in ms. Have to convert from uS to mS.
    case 80: statusValue = lowByte(injectors_context.injectors[3].PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
    case 81: statusValue = highByte(injectors_context.injectors[3].PW); break; //Pulsewidth 4 multiplied by 10 in ms. Have to convert from uS to mS.
