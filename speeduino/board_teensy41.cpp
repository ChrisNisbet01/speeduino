...
void initBoard()
{
    ...
    */
    ...
    vvt_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (2U * configPage6.vvtFreq * 2U)); //Converts the frequency in Hz to the number of ticks (at 2uS) it takes to complete 1 cycle
      ...
      fan_pwm_max_count = (uint16_t)(MICROS_PER_SEC / (2U * configPage6.fanFreq * 2U)); //Converts the frequency in Hz to the number of ticks (at 2uS) it takes to complete 1 cycle
    #endif
...
void setTriggerHysteresis()
{
  ...
  //Primary trigger
  p = digital_pin_to_info_PGM + Trigger.pin;
  *(p->reg + 1) &= ~(p->mask); // TODO: atomic
  *(p->pad) = padConfig;
  *(p->mux) = 5 | 0x10;

  //Secondary trigger
  p = digital_pin_to_info_PGM + Trigger2.pin;
  *(p->reg + 1) &= ~(p->mask); // TODO: atomic
  *(p->pad) = padConfig;
  *(p->mux) = 5 | 0x10;
}
