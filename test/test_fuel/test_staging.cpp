...
void test_Staging_4cyl_Auto_Inactive(void)
{
  ...
  injectors_context.injector(injChannel1).PW = testPW; //Over open time but below the pwLimit set below
  ...
  TEST_ASSERT_EQUAL(6000, injectors_context.injector(injChannel1).PW);
  TEST_ASSERT_EQUAL(6000, injectors_context.injector(injChannel2).PW);
  TEST_ASSERT_EQUAL(0, injectors_context.injector(injChannel3).PW);
  TEST_ASSERT_EQUAL(0, injectors_context.injector(injChannel4).PW);
  TEST_ASSERT_FALSE(BIT_CHECK(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE));
}
...
void test_Staging_4cyl_Table_Inactive(void)
{
  ...
  //PW 1 and 2 should be normal, 3 and 4 should be 0 as that testPW is below the pwLimit
  //PW1/2 should be (PW - openTime) * staged_req_fuel_mult_pri = (3000 - 1000) * 3.0 = 6000
  TEST_ASSERT_EQUAL(7000, injectors_context.injector(injChannel1).PW);
  TEST_ASSERT_EQUAL(7000, injectors_context.injector(injChannel2).PW);
  TEST_ASSERT_EQUAL(0, injectors_context.injector(injChannel3).PW);
  TEST_ASSERT_EQUAL(0, injectors_context.injector(injChannel4).PW);
  TEST_ASSERT_FALSE(BIT_CHECK(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE));
}
...
void test_Staging_4cyl_Auto_50pct(void)
{
  ...
  TEST_ASSERT_EQUAL(pwLimit, injectors_context.injector(injChannel1).PW); //PW1/2 run at maximum available limit
  TEST_ASSERT_EQUAL(pwLimit, injectors_context.injector(injChannel2).PW);
  TEST_ASSERT_EQUAL(9000, injectors_context.injector(injChannel3).PW);
  TEST_ASSERT_EQUAL(9000, injectors_context.injector(injChannel4).PW);
...
void test_Staging_4cyl_Auto_33pct(void)
{
  ...
  TEST_ASSERT_EQUAL(pwLimit, injectors_context.injector(injChannel1).PW); //PW1/2 run at maximum available limit
  TEST_ASSERT_EQUAL(pwLimit, injectors_context.injector(injChannel2).PW);
  TEST_ASSERT_EQUAL(6000, injectors_context.injector(injChannel3).PW);
  TEST_ASSERT_EQUAL(6000, injectors_context.injector(injChannel4).PW);
  TEST_ASSERT_TRUE(BIT_CHECK(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE));
}
...
void test_Staging_4cyl_Table_50pct(void)
{
  ...
  TEST_ASSERT_EQUAL(4000, injectors_context.injector(injChannel1).PW);
  TEST_ASSERT_EQUAL(4000, injectors_context.injector(injChannel2).PW);
  TEST_ASSERT_EQUAL(2500, injectors_context.injector(injChannel3).PW);
  TEST_ASSERT_EQUAL(2500, injectors_context.injector(injChannel4).PW);
  TEST_ASSERT_TRUE(BIT_CHECK(currentStatus.status4, BIT_STATUS4_STAGING_ACTIVE));
}